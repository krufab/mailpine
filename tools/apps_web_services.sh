...
function process_web_services() {
  ...
  cp "${APP_DIR}/roundcubemail/config.inc.template.php" "${APP_DIR}/pa/config.inc.php"
...
function process_web_nginx() {
  ...
  (
    ...
    for WEB_SERVICE_JSON in "${WEB_SERVICES_JSON[@]}"; do
      ...
      NAME="$(echo -n "${WEB_SERVICE_JSON}" | jq -r -c '.name')"
